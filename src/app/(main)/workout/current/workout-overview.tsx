...
/** `heights` (3540): the row height an unmeasured row is taken to be. */
...
export function WorkoutOverview({
  ...
  // The hold, the drag and the rows moving out of its way are the shared
  // gesture (`rowDragStart` 3538, `rowDragMove` 3552, `rowDragEnd` 3574).
      ...
      <div data-overview-body="">
        ...
        {exercises.map((exercise, index) => {
          ...
          return (
            <section
              key={exercise.id}
              {...rowGesture(reorder.rowProps(index), arriving)}
              data-overview-row=""
...
/**
 * An `add_exercise` the server has not acknowledged moves out of the way of a
 * row dragged past it, but cannot be taken hold of itself.
 */
function rowGesture(props: ReorderRowProps, arriving: boolean) {
  if (!arriving) return props;
  const { ref, style, ...rest } = props;
  return {
    ref,
    style,
    "data-reorder": rest["data-reorder"],
    "data-shifted": rest["data-shifted"],
  };
}
