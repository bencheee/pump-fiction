-- T-016: correct workout position renumbering.
-- The T-014 function renumbered workout_sets/workout_exercises through a
-- temporary negative-position update, which violates the positive-position
-- check constraints whenever sibling rows remain. Replace it with the
-- accepted offset-above-maximum technique already used by split reordering.
...
returns table (kind text, acknowledged_command_id uuid, acknowledged_workout_id uuid, expected_revision bigint, resulting_revision bigint)
language plpgsql
security invoker
set search_path = ''
as $$
