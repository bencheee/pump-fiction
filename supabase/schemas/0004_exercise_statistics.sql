-- T-033: the raw exercise performances behind S15 and S16.
--
-- These functions read and shape; they derive nothing. Personal records,
-- eligibility, and chart series are product rules and live in the History
-- domain, where they are testable without a database, as ADR-0020 requires of
-- chart data.
--
-- Both group by `exercise_identity_id`, never by the live `exercise_id`, so an
-- exercise keeps one history after its definition is deleted under ADR-0024.
...
-- The stored sets of one occurrence, ordered, as the domain shapes them.
create or replace function public.workout_set_snapshots(p_workout_exercise_id uuid)
returns jsonb
language sql
stable
security invoker
...
revoke execute on function public.list_exercise_history() from public, anon, authenticated;
revoke execute on function public.workout_set_snapshots(uuid) from public, anon, authenticated;
revoke execute on function public.get_exercise_performances(uuid) from public, anon, authenticated;

grant execute on function public.list_exercise_history() to service_role;
grant execute on function public.workout_set_snapshots(uuid) to service_role;
grant execute on function public.get_exercise_performances(uuid) to service_role;
