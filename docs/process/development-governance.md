...
- If the commit changes after confirmation, its prior test approval no longer covers the changed scope; obtain a new explicit confirmation before testing it.
...
Per [ADR-0021](../decisions/0021-delivery-and-evidence-commit-model.md), review, approval, and tests bind to the exact Task delivery commit, while narrow evidence commits durably record later events. Feature tests include unit, integration, end-to-end, application execution for behavioral validation, and manual feature scenarios. Formatting, linting, type checking, compilation/build, and documentation-link validation are permitted static checks; they do not authorize feature testing.
