...
Remove archiving from the data model and the application boundary and replace it with real deletion that never removes History. `ADR-0024` records the decision, the History-preservation mechanism, and the criteria it changes.
...
- Every screen change, delivered by `T-022`
...
- [ ] Deleting the last split of the current program is rejected with an explanatory failure.
...
- [ ] Extend pgTAP and repository integration assertions, including History survival after each deletion, without running them.
