...
Saving an exercise, program, or split closes the screen: the app returns to the parent screen and confirms the save with a toast there. A failure keeps the screen open, shows its field errors, and reports the failure as a toast. The parent screens are the Exercises list, the Programs list, and the owning program's edit screen. A form reports `Unsaved changes` only while it actually differs from the state it opened with, and carries an Unsaved badge in its top bar while it does. Since [ADR-0032](../decisions/0032-redesigned-mobile-visual-language.md) the save itself is picked in the screen's `···` panel and committed with Continue, together with every other action the screen offers. The implementation contract is in [`../architecture/mobile-ui-foundation.md`](../architecture/mobile-ui-foundation.md#definition-form-save-contract).
...
The workout is worked through one set at a time, which [ADR-0032](../decisions/0032-redesigned-mobile-visual-language.md) decided.

The header is one row beyond the safe-area inset: the pause toggle, the live clock, and the way into the overview. A segment per set runs across the whole workout under it — recorded, current, outstanding — and tapping one jumps to that set. Pausing says in words that the active duration is not counting.

The stage names the set, its position, and the prescription it came from, then the exercise, then the value on two wheels: load where the mode has one, a Bodyweight tag where it does not, and reps or seconds. A band row appears only when the mode needs one. The wheels hold a draft until the set is logged, and a set with no values starts from the set before it, then the previous performance, then the lowest planned count.

The primary action logs the set and plays the banked-set flash before the next set arrives; once everything is recorded it reads Review & finish instead. Below it sit Last, Note, More, and the round check that opens the review. **Up next** names the set after the one on screen, not the first one outstanding.

Between two exercises a handoff screen names what was finished and what waits. When every planned set is recorded, a completion screen stands before the terminal command.

Last time, the note, the exercise actions, and the review are full-screen panels. The exercise actions panel carries the definition's optional addition, adding a set, and both removals; a populated removal still asks first. The review keeps Complete workout, Continue workout, and the separately confirmed Discard workout together, and opens from the snapshot already on screen without a route or database read. Start and finish persistence cover the viewport with a progress layer.

The overview shows every exercise with what it has recorded, reorders by holding a card — arrow keys move a focused card too — removes one, adds from the library, and returns to the set in hand.

Restoring a workout shows no banner: the restored workout itself is the evidence. The persistent exercise note stays read-only and is read in the note panel beside the note kept with this occurrence.

Today carries no weight or measurement card: both are recorded in Body.
...
The first tab of the Body destination. Provide latest value, weekly average and change, recorded-days count, time-range selector, chart, entry list, and the way to record one.

The two summaries are stat cards: the latest weigh-in with its date and its change from the one before it, and this week with its average, its change from last week or the unavailable state, its `n/7` count, and whether it is provisional or final. A week with no weigh-in yet says so rather than showing a zero. The range selector offers week, month, quarter, and year, and opens on the month. The chart's reading follows the selected bar, a sentence sits under it, and a collapsible list repeats every weigh-in; the weekly averages keep their own list beside it, with their spans.

**Add** opens the entry panel: a month grid for the date, the value beside it, and the `···` panel to save. Any past day is allowed and no future day is offered. Each row opens the entry screen for its date, which corrects the value it already holds; that screen's save, validation, and outcome cue is the first row of the sticky action bar, and **Delete Entry** sits behind the destructive confirmation that names what recalculates.
...
The detail names the unit under the measurement's name, leads with the latest value on an accent block, then gives the latest change and the total change as stat cards, each saying so plainly when there is nothing to compare against. Its range selector offers month, quarter, year, and all, and opens on all. **Record measurement** sits in the sticky action bar and opens the same entry panel the weight tab uses, one measurement at a time. Each row opens the entry screen for its date, which mirrors the weight one — the cue in the first row of the sticky action bar, and deletion behind the destructive confirmation.
