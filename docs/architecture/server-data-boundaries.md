...
`T-055` removes three avoidable hosted-data paths without changing that boundary. `get_today_view` includes the ordered exercise prescriptions for every current-program split, replacing the page's per-split application calls. `start_workout_and_get_current` composes the existing transactional start and current-workout hydration inside one database invocation, so the repository no longer follows the start RPC with another network request. Current Workout loads only its required workout aggregate at the server boundary; the thin `listExercisesAction` reads the exercise library when the Add Exercise sheet actually opens. The ordinary finish review is derived from the already loaded client snapshot and opens without server work. Selecting an outcome then drains pending commands and uses the existing thin current-workout action once under the blocking layer before sending its terminal command through the durable endpoint; this synchronization prevents a reload racing the prior page's acknowledgement from sending a stale revision. The direct finish route remains available as a server-hydrated fallback.
