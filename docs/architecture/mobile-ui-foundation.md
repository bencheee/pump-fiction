...
- Every interactive primitive has at least a 44 by 44 CSS pixel target; primary and secondary controls use the larger frozen values where applicable. The active-workout set-entry inputs are the one compact exception: their visible and interactive field height is 32 CSS pixels, accepted for the repeated dense data-entry grid while labels and spacing keep adjacent targets distinct.
...
`T-054` reads each available split through the existing program application boundary beside that aggregate and passes only its serializable prescriptions into the client. Today therefore renders the selected split's ordered exercise preview directly below Start Workout without widening `TodayView`, changing `get_today_view`, or introducing a second database read path.
...
- `T-054` keeps those targets and commands but compacts their presentation: each exercise card is independently collapsible and open by default, has no decorative drag handle, and places its visually small up/down/remove icons in the title row while retaining 44-pixel hit areas. Set title rows place the optional-addition control immediately before the icon-only removal action, repeated numeric inputs are 32 pixels high, exercise guidance uses the warning-yellow token, and Add Set is unbordered green text aligned right. Last time carries its date in the heading and uses one reps-first line per set.
