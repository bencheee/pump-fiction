...
- The round lower-right check action opens an application-owned review sheet synchronously from the active client snapshot; it performs no route or database read. The sheet derives active duration, exercise count, recorded sets, and — for split-sourced workouts only — the planned sets left without values, listing those prescribed rows. Complete, Save as Incomplete, and confirmed Discard still deliver `finish_workout` through the same queue and return to Today when acknowledged. Start and finish transitions render one full-viewport busy layer above the shell until their asynchronous persistence/navigation completes.
