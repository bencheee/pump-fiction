...
The History layout under `/history` owns a subsection navigation. The shared `src/shared/ui/subsection-navigation.tsx` gives `/body` the same behavior rather than duplicating it: both sit inside the main shell, keep bottom navigation visible, and mark the current tab with the filled pill that slides to it and with `aria-current`, which carries the same fact to anything that cannot see the pill.

The bar and the sliding pill live in the layout, not in the page, because the layout is what stays mounted while the page under it changes; a pill rendered per page would have nothing to slide from. History's three tabs, and Body's two, are therefore one screen each: the shared `TabbedFrame` in `src/shared/ui/tabbed-frame.tsx` holds the title and the bar in the destination's layout and hands the panel's own entry direction down to the page, the page contributes the count and the scroll region, and a grid puts the four back in the order the design draws them. Moving between the three takes no screen transition — `useScreenAnimation` answers for them under one key — and every other route under `/history` is a screen of its own with its own top bar, which the frame steps out of the way for.
...
Every chart is the shared bar chart card in `src/shared/ui/bar-chart.tsx`, with a `body` and a `measure` variant for Body's two charts. It receives finished points, each with its formatted value and the height it stands at, and computes nothing; the screen's own scale decides the heights. A press on a bar moves the reading above the chart, so nothing depends on hover, and the summary sentence under it and its values list, or the entries under the measurement chart, carry the same data, as [ADR-0020](../decisions/0020-mobile-ui-charting-and-quality-tooling.md) requires. The Recharts line in `src/features/history/ui/progress-chart.tsx` has no screen left; it and the dependency go with the test pass, which updates ADR-0020.
