...
Every weight value is derived at read time in the History domain beside the exercise and split statistics. `src/features/history/domain/weight.ts` holds the calendar-week grouping, weekly average and change, provisional rule, individual change, and chart series as pure functions of stored entries and the configured local date; `weight-validation.ts` holds the entry rules. No table caches a weekly average, so correcting a historical weigh-in is reflected immediately everywhere.

Body measurements follow the same pattern in `body.ts` and `body-validation.ts`: one read returns every measurement type with everything recorded for it, and the domain derives list summaries, latest and total change, and chart series. Both changes are unavailable rather than zero until a second measurement exists, and no series or summary carries a direction, because which way is progress depends on the measurement and goal.
...
These calculations live in the History domain rather than in SQL: eligibility, a set's comparison category, every personal record, the latest eligible performance, and chart series are pure functions over stored performances, so each product rule is testable without a database. The database supplies raw performances and nothing more. Splits follow the same shape: one read returns every saved split-sourced workout with its identity snapshots, live template names where they still exist, and the names it carries regardless; counts, the five duration statistics, program filter, and duration series are derived in the domain and grouped by identity alone. Chart ranges are trailing windows ending on the configured local date, and `all` is unbounded.
