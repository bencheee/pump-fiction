...
`T-041` does the same for body measurements in `body.ts` and `body-validation.ts`: one read returns every measurement type with everything recorded for it, and the domain derives the list summaries, the latest and total change, and the chart series from that. Both changes are unavailable rather than zero until a second measurement exists, and no series or summary carries a direction, because which way is progress depends on the measurement and the goal.
