...
`T-031` adds the `0006_workout_history` pgTAP suite and a workout-History repository integration test to that set, `T-033` adds `0007_exercise_statistics` and an exercise-statistics repository integration test, `T-035` adds `0008_split_statistics` and a split-statistics repository integration test, and `T-038` adds `0009_weight_operations` and a weight repository integration test. Both weight suites work on dates far in the past and remove those rows again, because a weigh-in is unique per local date and must not collide with a real one. The suite creates and finishes its own workouts, so it shares the precondition below.
