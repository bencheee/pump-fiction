...
Set rows carry two constraints with different jobs: the shape check accepts any partially entered set, including a band mode without a strength, while the confirmation check requires the complete value set, now explicitly including a band strength for band modes. `T-025` corrected the shape check after a partial band entry was rejected mid-workout.
