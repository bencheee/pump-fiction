...
[ADR-0003](0003-history-information-architecture.md) originally grouped every historical record and statistic under one History destination, so weight and body measurements were two of its five subsections.
...
It revises five baseline criteria:
...
The shell, mobile-UI-foundation browser spec, and route inventory all assert the resulting five-destination rule.

Both design packages predate this decision and carry a supersession note rather than being rewritten.
