...
Run the complete recorded `T-008` test plan from the beginning against exact approved replacement `9dab09e2520be061a1c8832526bfa2bdb19efec3` in a fresh isolated worktree.

## Implementation status

`T-005`, `T-006`, and `T-007` are `Done`; the application baseline, declarative local schema, generated database types, server-only client and import direction, serializable operation contracts, app-settings repository/query/mutation boundaries, and their approved verification are complete. The latest `T-008` lockfile-only replacement is statically verified and `Approved`; its complete authorized verification is next, while feature UI behavior and deployment configuration do not exist yet.
