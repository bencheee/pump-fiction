...
Deliver `T-018`, the first Task of `F-011` MVP Experience Corrections, which the Owner made `Ready` on 2026-09-05. `F-011` records the nine corrections the Owner raised on 2026-09-05 after using the delivered application; three of them change accepted decisions and locked criteria and therefore create `ADR-0023`, `ADR-0024`, and `ADR-0025`. `F-008` History and Statistics starts only after `F-011` is complete.
