...
Repeat the complete T-009 verification against exact approved replacement `746985808e7e587b1aa4366d00b133f121d6cd73` in an isolated worktree.

## Implementation status

`T-005`, `T-006`, `T-007`, and `T-008` are `Done`; the application baseline, declarative local schema, generated database types, server-only boundaries, active-workout transactional command path, IndexedDB FIFO outbox, restore/replay recovery, and their approved verification are complete. `T-009` replacement is `Approved` for testing; domain feature UI behavior and deployment configuration do not exist yet.
