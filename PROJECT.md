...
- **Last updated:** 2026-09-05T13:00:15+02:00
...
User reviews delivery commit `dfd6d7a9587740717af43ef23f33e3d545a20c4c` for [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) and decides on approval; database and application verification stays blocked until then. The proposed current-program mechanism in [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) and the per-set control placement in [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) still need Owner confirmation before those Tasks become `Ready`.
...
- [`F-011`](docs/project/features/F-011-mvp-experience-corrections.md) — MVP Experience Corrections (`3/6 Tasks Done`) — Owner confirmed readiness
  ...
  - [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) — `In Review` — Claude Code primary agent — `2026-09-05T12:42:09+02:00` — deliver one reviewable commit (correction 3)
