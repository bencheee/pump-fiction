...
- **Last updated:** 2026-09-06T11:01:09+02:00
...
- **Implementation:** `F-004` through `F-007` and `F-011` through `F-014` complete; `T-031` through `T-036` of `F-008` are `Done` and `T-037` is `In Progress`
...
[`T-037`](docs/project/tasks/T-037-repair-stale-browser-specs.md) — Repair the browser specs left stale by the archiving removal — `In Progress`, Executor Claude Code primary agent, last change 2026-09-06T11:01:09+02:00. It is the last `F-008` Task: two older specs still call the archiving artifacts `T-021` removed, so `npm run test:browser` cannot run as a whole until they are repaired.
...
Deliver `T-037` for the Owner's one approval; its verification runs the whole browser suite.

## Now

- [`T-037`](docs/project/tasks/T-037-repair-stale-browser-specs.md) — Repair the browser specs left stale by the archiving removal — `In Progress` — Claude Code primary agent — 2026-09-06T11:01:09+02:00 — next: deliver for review.
