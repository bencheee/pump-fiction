...
- **Last updated:** 2026-09-05T19:07:51+02:00
...
User reviews delivery commit `75fd3d78d71c599cfcd54026080e51c79fade3af` for [`T-025`](docs/project/tasks/T-025-allow-partial-band-set-entry.md) and decides on approval; pgTAP verification stays blocked until then. The proposed current-program mechanism in [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) and the per-set control placement in [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) still need Owner confirmation before those Tasks become `Ready`.
