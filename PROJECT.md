...
- **Last updated:** 2026-09-05T13:04:55+02:00
...
User reviews the `T-021` test-only replacement `a049287a74a7dccf5ba2146bea09671913dca9d7` and decides on fresh approval; the complete verification then restarts from the beginning. The proposed current-program mechanism in [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) and the per-set control placement in [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) still need Owner confirmation before those Tasks become `Ready`.
