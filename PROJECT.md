...
- **Last updated:** 2026-09-05T12:36:44+02:00
...
User reviews delivery commit `c48cbdcaf0bc348888ef2e1eff7193269a6f049c` for [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) and decides on approval; component verification stays blocked until then. The proposed current-program mechanism in [`T-021`](docs/project/tasks/T-021-replace-archiving-with-deletion-in-data.md) and the per-set control placement in [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) still need Owner confirmation before those Tasks become `Ready`.
...
- [`F-011`](docs/project/features/F-011-mvp-experience-corrections.md) — MVP Experience Corrections (`2/7 Tasks Done`) — Owner confirmed readiness
  ...
  - [`T-020`](docs/project/tasks/T-020-derive-per-set-load-from-definition.md) — `In Review` — Claude Code primary agent — `2026-09-05T12:33:15+02:00` — deliver one reviewable commit (correction 7)
